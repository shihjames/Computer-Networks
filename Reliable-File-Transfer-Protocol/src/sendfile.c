		...
		while (i < 600)
		{
			...
			if (lastRound && totalBytesLeft > MESSAGE_SIZE)
			{
				...
				do
				{
					...
					printf("size: %d", sizeof(packet.message));
			...
			{
				struct Packet packet;
				packet.size = totalBytesLeft;
				printf("totalBytesLeft: %d\n", totalBytesLeft);
				packet.checksum = 0;
				packet.seqNum = seq;
				strcpy(packet.message, "");
				...
				printf("size final: %d\n", sizeof(packet));
				printf("size final message: %d\n", sizeof(packet.message));
				packet.checksum = cksum((unsigned short *)&packet, sizeof(packet) / sizeof(unsigned short));
				htonPkt(&packet);
				do
				{
