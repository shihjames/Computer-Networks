...
int main(int argc, char **argv)
{
	...
	while (true)
	{
		...
		if (recvByteCnt != -1)
		{
			...
			// calculate checksum
			int cksumRes = cksum((unsigned short *)&recvPacket, sizeof(recvPacket) / sizeof(unsigned short));
			printf("cksum: %d\n", cksumRes);
			...
			else
			{
				...
				if (recvSeqNum != 65535)
				{
					...
					{
						int length = sizeof(recvPacket.message);
						printf("length: %d, message size: %d\n", length, recvPacket.size);
						if (recvPacket.size == 1400)
						{
						...
						else
						{
							...
							fwrite(recvPacket.message, 1, recvPacket.size, file);
							int retVal = recvSeqNum;
							htons(&retVal);
							sendto(sock, &retVal, sizeof(int), 0, (const struct sockaddr *)&sin, sizeof(sin));
							curSeqNum++;
